 ...
 * @arg_vect: the argument vector from main()
 *
 * Return: 0 on success, 1 on error, or error code
 */
int hsh(info_t *info_struct, char **arg_vect)
{
	ssize_t z = 0;
	int builtin_ret = 0;
	...
	while (z != -1 && builtin_ret != -2)
	{
		...
		if (interactive(info_struct))
			_puts("$ ");
		_eputchar(BUF_FLUSH);
		z = get_input(info_struct);
		if (z != -1)
		{
			set_info(info_struct, arg_vect);
...
int find_builtin(info_t *info_struct)
{
	...
	for (x = 0; builtintbl[x].type; x++)
		if (_strcmp(info_struct->argv[0], builtintbl[x].type) == 0)
		{
			info_struct->line_count++;
			built_in_ret = builtintbl[x].func(info_struct);
			break;
		}
	return (built_in_ret);
}
...
void find_cmd(info_t *info_struct)
{
	...
	for (x = 0, k = 0; info_struct->arg[x]; x++)
		if (!is_delim(info_struct->arg[x], " \t\n"))
			k++;
	if (!k)
		return;
